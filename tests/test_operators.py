...
def test_sigmoid(a: float) -> None:
    sig = sigmoid(a)
    assert 0.0 <= sig <= 1.0, "Sigmoid not in range [0, 1]"
    assert_close(1.0 - sigmoid(a), sigmoid(-a))
    assert_close(sigmoid(0), 0.5)
    assert sigmoid(a) >= sigmoid(a - 0.1), "Sigmoid is not strictly increasing"
...
def test_symmetric(a: float, b: float) -> None:
    assert_close(mul(a, b), mul(b, a)), "Multiplication is not symmetric"


@pytest.mark.task0_2
@given(small_floats, small_floats, small_floats)
def test_distribute(x: float, y: float, z: float) -> None:
    assert_close(mul(z, add(x, y)), add(mul(z, x), mul(z, y))), "Distributive property failed"


@pytest.mark.task0_2
@given(small_floats)
def test_other(a: float) -> None:
    assert_close(neg(neg(a)), a), "Double negation failed"
